...
function goDelete(userId){
	if(window.confirm("确定要删除数据吗?")){
		location.href="${APP_PATH}/user/delete?userId="+userId;
	}
}
</script>
...
<body>
    ...
    <div class="tools">
    	...
    	<ul class="toolbar">
        <li class="click">
        	<a href="${APP_PATH}/user/goAddOrUpdate">
	        	<span>
    ...
    <table class="tablelist">
        ...
        <th>用户编号<i class="sort"><img src="${APP_PATH}/images/px.gif" /></i></th>
        <th>账号</th>
        <th>姓名</th>
        <th>邮箱</th>
        <th>电话</th>
        ...
        <c:forEach items="${pageModel.list }" var="user">
	        ...
	        <td>${user.email }</td>
	        <td>${user.phone }</td>
	        <td><a href="${APP_PATH}/user/goAddOrUpdate?userId=${user.userId }" class="tablelink">修改</a>
    ...
    <div class="pagin">
    	<form action="${APP_PATH}/user/queryPage" method="post" id="pager">
